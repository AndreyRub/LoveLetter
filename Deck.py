...
class Deck:
    ...
    def get_descriptions(self,cards_style="Japanese"):
        if cards_style=="Japanese":
            ...
            descriptions = ['Batman(5) - Guess a player\'s hand',
                            ...
                            'Harley Quinn(1) - Discard if caught with Poison Ivy or Two-Face',
    ...
    def __init__(self, num_of_unused_cards=1,cards_style = "Japanese", shuffle_order=[]):
    ...
    def shuffle(self):
        ...
        if self.shuffle_order == [] or len(self.shuffle_order)!=len(self.cards):
            random.shuffle(self.cards)
        else:
            self.cards = [self.cards[ind] for ind in self.shuffle_order]
